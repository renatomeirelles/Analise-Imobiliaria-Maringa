...
with col_chart:
    ...
    fig = None
    # Formatter para eixos de valores em R$
    ...
    if grafico_tipo == "Histograma":
        ...
        ax.hist(df_filtrado[coluna_valor], bins=30, color="#00CED1", edgecolor="white")
        ax.set_title(f"Distribuição de {tipo_estatistica}", color="white", fontsize=12, pad=8)
        ax.set_xlabel("Valor (R$)", color="white")
        ax.set_ylabel("Quantidade de imóveis", color="white")
        ax.tick_params(colors="white")
        ...
        ax.set_facecolor("#111111")
        media_bairro.plot(kind="barh", ax=ax, color="#00CED1")
        ax.set_title(f"Média de {tipo_estatistica} por bairro (top 15)", color="white", fontsize=12, pad=8)
        ax.set_xlabel("Valor médio (R$)", color="white")
        ax.tick_params(colors="white")
        ax.xaxis.set_major_formatter(currency_formatter)
        # Forçar a exibição dos nomes dos bairros
        ax.set_yticks(range(len(media_bairro.index)))
        ax.set_yticklabels(media_bairro.index, color="white")
        ax.invert_yaxis()
        fig.tight_layout()
        ...
        ax.set_facecolor("#111111")
        sns.boxplot(data=df_filtrado, x="Tipo", y=coluna_valor, ax=ax, palette="Set2")
        ax.set_title(f"Distribuição de {tipo_estatistica} por tipo de imóvel", color="white", fontsize=12, pad=8)
        ax.set_xlabel("Tipo de imóvel", color="white")
        ax.set_ylabel("Valor (R$)", color="white")
        ax.tick_params(axis="x", colors="white", rotation=30)
        ax.tick_params(axis="y", colors="white")
    ...
    if fig is not None:
        st.pyplot(fig, clear_figure=True)
# =========================
# Padronização de estilo dos gráficos (garantia de legibilidade)
# =========================
def style_axes(ax):
...
# Exemplo de uso:
# depois de criar ax em cada gráfico, chame:
# style_axes(ax)
# =========================
...
st.markdown(
    ...
    .st-emotion-cache-1jicfl2, .st-emotion-cache-13dfmoy, .st-emotion-cache-1v0mbdj {
        margin: 0 !important;
        padding: 0 !important;
    }
